  ...
  template: `
    <header class="hero" id="home">
      <div class="hero-content">
        <h1 class="hero-title" [innerHTML]="getFormattedTitle()"></h1>
  ...
  styles: [`
    .hero {
      ...
      background: linear-gradient(rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.3)),
                  url('/yc-assets/Download_X.jpg') center/cover no-repeat;
      display: flex;
    ...
    ::ng-deep .hero-title .ampersand {
      color: #606E41 !important;
    }
...
export class HeroComponent implements OnInit, AfterViewInit {
  ...
  getFormattedTitle(): string {
    const title = this.settings?.siteName || 'YC INTERIOR & BUILDERS';
    // Replace & with a span that has the green color
    return title.replace(/&/g, '<span class="ampersand">&amp;</span>');
  }
}
