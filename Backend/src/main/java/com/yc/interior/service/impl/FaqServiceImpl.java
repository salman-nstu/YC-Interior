...
public class FaqServiceImpl implements FaqService {
    ...
    public FaqResponse update(Long id, FaqRequest request) {
        ...
        if (request.getDisplayOrder() != null && !request.getDisplayOrder().equals(entity.getDisplayOrder())) {
            ...
            if (oldOrder != null && (newOrder >= 0 && newOrder <= 4)) {
                // If moving to a different order, shift items
                if (newOrder < oldOrder) {
                    // Moving up (lower order number)
                    repository.findAll().stream()
                            .filter(f -> !f.getId().equals(id) && f.getDisplayOrder() != null && 
                                    f.getDisplayOrder() >= newOrder && f.getDisplayOrder() < oldOrder)
                            .forEach(f -> {
                                f.setDisplayOrder(f.getDisplayOrder() + 1);
                                repository.save(f);
                            });
                    ...
                    repository.findAll().stream()
                            .filter(f -> !f.getId().equals(id) && f.getDisplayOrder() != null && 
                                    f.getDisplayOrder() > oldOrder && f.getDisplayOrder() <= newOrder)
                            .forEach(f -> {
                                f.setDisplayOrder(f.getDisplayOrder() - 1);
                                repository.save(f);
                            });
